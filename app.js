...
const records = { issues: {}, guest_requests: {}, users: {}, settings: {}, manual_notification_logs: {} };
...
function render(view=currentView){
  if (!canAccess(view)) view='dashboard';
  currentView=view;
  const views={dashboard,rooms:roomsView,issues:()=>operationalView('issues'),requests:()=>operationalView('guest_requests'),reports:reportsView,users:usersManagementView,notifications:notificationsView,settings:settingsView};
  app.innerHTML=(views[view]||dashboard)();
  const activeTitle={dashboard:'Genel Bakış',rooms:'Odalar',issues:'Sorunlar',requests:'Misafir Talepleri',reports:'Raporlar',users:'Kullanıcılar',notifications:'Özel Bildirimler',settings:'Otel Ayarları'}[view];
  title.textContent=activeTitle;
