...
function roomCards(sourceRooms=rooms){ return sourceRooms.map(([number,status,key,floor,,guestName,occupied,doNotDisturb]) => { const activity=roomActivity(number); const hasCleaningRequest=hasActiveCleaningRequest(activity.requests); const statusChoices=allowedRoomStatuses(currentRole,key).filter(nextKey=>nextKey!==key); const canAddRequest=currentRole!=='housekeeper'; const isHousekeepingStart=currentRole==='housekeeper'&&statusChoices.includes('progress')&&key==='dirty'&&(!occupied||hasCleaningRequest); return `<article class="room-card ${isHousekeepingStart?'housekeeping-room-card':''}" data-room-interaction data-status="${esc(key)}" data-room="${esc(number)}" data-action="room-details"><div class="room-card-top"><div><h3>${esc(number)}</h3><p>${esc(floor)}</p>${guestName?`<p class="room-guest-name">${esc(guestName)}</p>`:''}</div><span class="room-status ${esc(key)}"><i></i>${esc(status)}</span></div><div class="room-alerts">${occupied?'<span class="room-alert occupied-alert">Dolu</span>':'<span class="room-alert vacant-alert">Boş</span>'}${key==='dirty'&&occupied&&!hasCleaningRequest?'<span class="room-alert request-alert">Temizlik talebi yok</span>':''}${hasCleaningRequest?'<span class="room-alert request-alert">Temizlik talebi</span>':''}${doNotDisturb?.enabled?`<span class="room-alert dnd-alert">Rahatsız Etmeyin: Açık</span>`:''}${activity.issues.length?`<span class="room-alert issue-alert">△ ${activity.issues.length} sorun</span>`:''}${!activity.requests.length&&!activity.issues.length&&!doNotDisturb?'<span class="room-clear">Açık kayıt yok</span>':''}</div><div class="room-dnd-status">${esc(doNotDisturbText(doNotDisturb))}</div><div class="room-card-actions ${isHousekeepingStart?'has-housekeeping-start':''}">${canAddRequest?`<button class="mini-button room-request-action" data-action="add-room-request" data-room="${esc(number)}"><span aria-hidden="true">＋</span> Talep ekle</button>`:''}${statusChoices.length&&isHousekeepingStart?`<div class="room-status-choices ${isHousekeepingStart?'housekeeping-start-choice':''}" aria-label="Yeni oda durumu">${statusChoices.map(nextKey=>`<button class="room-status-choice status-choice-${esc(nextKey)}" data-action="set-room-status" data-room="${esc(number)}" data-status-key="${esc(nextKey)}">Temizliğe başla</button>`).join('')}</div>`:''}</div>${currentRole==='admin'?`<div class="room-admin-actions"><button class="mini-button room-admin-action" data-action="edit-room" data-id="${esc(number)}">Düzenle</button><button class="mini-button room-admin-action" data-action="delete-room" data-id="${esc(number)}">Sil</button></div>`:''}</article>`; }).join(''); }
